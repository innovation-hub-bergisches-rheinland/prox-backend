...
class GetRecommendationsHandlerTest {
  ...
  GetRecommendationsHandler getRecommendationsHandler = new GetRecommendationsHandler(
      tagCollectionFacade, userProfileFacade, organizationFacade, projectFacade);
  ...
  void shouldNotHaveHigherScoreThanOne() {
    ...
    var projects = Instancio.ofList(ProjectDto.class)
        .size(100)
        .supply(field(ProjectDto::supervisors),
            () -> List.of(new ReadSupervisorDto(lecturer.userId(), lecturer.displayName())))
    ...
    when(userProfileFacade.findLecturersByIds(
        argThat(ids -> ids.contains(lecturer.userId())))).thenReturn(List.of(lecturer));
    ...
    assertThat(getRecommendationsHandler.handle(new RecommendationRequest(tagIds, List.of(), 10)))
        .satisfies(r -> {
          assertThat(r.projects())
              .isNotEmpty()
              .isSortedAccordingTo(
                  (p1, p2) -> p2.item().createdAt().compareTo(p1.item().createdAt()));
        });
  }
}